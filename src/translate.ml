...
let rec expr (e, sm, fm) = 
      ...
      Str i -> ("String", [Strg i])
        ...
        | Assign (id, value) -> if(fst (expr(value, sm, fm) ) = "" ) (*List type work around*)
                      then ( fst (expr(value, sm, fm) ), [Assign (id, (expr(value, sm, fm) ))])
                      else(
                        if(fst (expr(value, sm, fm) ) = (StringMap.find id sm) )
                        then ( fst (expr(value, sm, fm) ), [Assign (id, (expr(value, sm, fm) ))])
                        else raise (ParseError "type not match")
                      )
        ...
        | Mem(id, i) -> ("",[Mem (id, i)])
        | ListAssign(id, i, e) -> ("", [ListAssign (id, i, (snd (expr (e, sm, fm) ) ) )])
